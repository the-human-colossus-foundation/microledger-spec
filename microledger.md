...
* `Authentic Timestamp` - timestamp is necessary to assure about authenticity of the data. E.g. if the key was compromise knowing when the creation happen is the only way to find out if the data were created in authnetic way (data comes from of undisputed origin). 
